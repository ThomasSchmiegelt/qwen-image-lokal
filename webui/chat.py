"""Freitext in Bildparameter uebersetzen.

Ein kleines Sprachmodell aus dem lokal laufenden Ollama liest einen Satz wie
"mach mir 10 Illustrationen von einem Drachen als Zeichentrick, hochkant" und
gibt die Einstellungen zurueck, die die Oberflaeche sonst von Hand bekommt.

Der eigentliche Gewinn ist nicht das Ausfuellen der Felder, sondern die
Uebersetzung: Qwen-Image folgt englischen Bildbeschreibungen deutlich besser
als deutschen. Der Benutzer schreibt deutsch, das Modell schreibt den Prompt.

Nichts aus der Antwort wird ungeprueft uebernommen -- `sanitise()` laesst nur
...
from presets import CAMERAS, EFFECTS, FORMS, LIGHTS, STYLES, VIEWS, overrides
...
def system_prompt(has_images: int) -> str:
    ...
    veraltet, sobald in presets.py etwas dazukommt."""
    situation = (
...
# Qwen-Image folgt englischen Bildbeschreibungen deutlich besser. Wer deutsch
...
def translate(fields: dict[str, str], model: str | None = None) -> dict[str, str]:
    ...
    try:
        with urllib.request.urlopen(request, timeout=180) as response:
            raw = json.loads(THINK.sub("", json.load(response)["message"]["content"]).strip())
    except Exception:
        return {}
    if not isinstance(raw, dict):
        return {}
